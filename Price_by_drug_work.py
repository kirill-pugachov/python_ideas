...
#Объявляем переменные 69314 12077 92625 197577 79895 127389 127390 7168 365087 366694
drug_id=7168
geo_lat=50.391203
geo_lng=30.375645
quant=3000
...
def Scale_count_width(canvas_width, parsed_json):
    ...
    return Scale
#сохраняем в файл аптеки, которые не прошли по условию правильности цены +\- 3 сигма    
...
#рисуем линии цены на лек.форму по аптечно на канве
def Price_line_draw(parsed_json):
    I=0
...
#рисуем горизонтальные оси средних и медиан
def Horizontal_line_draw(parsed_json):
...
#забираем введенное пользователем значение новой drug_id
def Update(event):
    drug_id=New_drug_id.get()
    print(drug_id)
    canvas.delete('all')

    parsed_json=Get_data_from_api(drug_id, geo_lat, geo_lng, quant)
    Sorted_list(parsed_json)
    Price_line_draw(parsed_json)
    Horizontal_line_draw(parsed_json)
    root.title('Цены на препарат' + ' ' + str(drug_id)+' '+Lek_forma_name(drug_id))

#Забираем данные из API
def Get_data_from_api(drug_id, geo_lat, geo_lng, quant):
...
#печатаем в консоль параметры по ценам и аптекам
...
But_update=tk.Button(Frame_for_buttons, text='Обновить график')
But_update.bind("<Button-1>", Update)
But_update.bind("<Return>", Update)
But_update.pack(padx=5, pady=10, side='left')

But_get_result=tk.Button(Frame_for_buttons, text='Сохранить результат')
But_get_result.bind("<Button-1>", Get_result)
But_get_result.bind("<Return>", Get_result)
